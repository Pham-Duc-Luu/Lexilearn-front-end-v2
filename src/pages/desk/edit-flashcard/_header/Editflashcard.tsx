...
const EditFlashcard = (props: EditFlashcardProps) => {
  ...
  const [cardContent, setCardContent] = useState(props.cardContent);
  const text = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      setCardContent({ ...cardContent, text: truncatedContent });
  ...
  useEffect(() => {
    if (props.onCardContentChange && cardContent)
      props.onCardContentChange(cardContent);
  }, [cardContent]);
  return (

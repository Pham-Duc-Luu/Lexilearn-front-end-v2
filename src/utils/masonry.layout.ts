...
export function arrangeColumns<T>(
  ...
  const array = Array(number_of_columns).fill([]);
  const counterArray = Array(number_of_columns).fill(0);
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (array[minIndex]) array[minIndex] = [...array[minIndex], list[i]]; // Add the item to the smallest column
